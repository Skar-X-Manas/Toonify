...
def cartoonize_image(image_path, output_path, resize_factor=0.2, oil_paint_intensity=1):
    # Read the image
    ...
    # Apply oil paint effect using cv2.stylization
    oil_painted_img = cv2.stylization(cartoon, sigma_s=150, sigma_r=0.25)

    # Display the original and cartoonized images
    cv2.imshow('Original Image', resized_img)
    cv2.imshow('Cartoonized Image', cartoon)
    cv2.imshow('Oil Painted Image', oil_painted_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    # Save the cartoonized and oil painted image
    cv2.imwrite(output_path, oil_painted_img)

# Replace 'path/to/your/image.jpg' with the actual path to your image file
# Replace 'path/to/your/output_cartoon.jpg' with the desired output path for the cartoonized image
cartoonize_image('isha.jpg', 'output_cartoon_oil_painted.jpg')
